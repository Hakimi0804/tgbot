...
_log_upload() {
	...
	_LOG_PROGRESS+=("\`$(_log_date)\` \\- Uploading ${_TMP_FILE_PATH_NAME//_/\\_}$n")
	_log_editmsg
...
_log_for_five_sec() {
	...
	_LOG_PROGRESS+=("\`$(_log_date)\` \\- Waiting for device\\.\\.\\.$n")
	_log_editmsg
	adb wait-for-device
	_LOG_PROGRESS+=("\`$(_log_date)\` \\- Taking log \\| type: $_TYPE$n")
	_log_editmsg
	case $_TYPE in
	'all')
...
_log_link() {
	local _FILE_PATH=$1
	_LOG_PROGRESS+=("\`$(_log_date)\` \\- Creating gist$n")
	_log_editmsg
	local _LOG_URL=$(gh gist create <"$_FILE_PATH" | tail -n1)
	_LOG_PROGRESS+=("\`$(_log_date)\` \\- gist link: ${_LOG_URL//\./\\\.}$n")
	_log_editmsg
...
_log_purge() {
	local _FILE_PATH=$1
	_LOG_PROGRESS+=("\`$(_log_date)\` \\- Purging AutoPasteSuggestionHelper \\(contains clipboard content\\)$n")
	_log_editmsg
